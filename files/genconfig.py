...
def database_map(storage, value, path):
    ...
    re_matches = re.search(r'postgresql\+psycopg2://(?P<user>\w+):(?P<pswd>[^\@]+)@(?P<host>[^:/]+)(:(?P<port>\d+))?/(?P<name>.+)', old_value)

    coordinates = {
        "name": re_matches.group('name'),
        "user": re_matches.group('user'),
        "pswd": re_matches.group('pswd'),
        "host": re_matches.group('host'),
        "port": re_matches.group('port') if re_matches.group('port') else 5432
    }
...
def main():
    ...
    prop_names = [name for name in os.environ if name.startswith('CMS_')]
